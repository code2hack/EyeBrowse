...
Worker: Worker-v0.01. [Controlling R2 plan 5771447930](https://github.com/code2hack/EyeBrowse/issues/8#issuecomment-5771447930), sections 5–8. Baseline `2a926733`. T03 adds two Kotlin instrumented companions and fixture observations; production Phone/RG, wire messages, canonical v2 command IDs, durable ordinals and admission remain unchanged.
...
Host inventory remains the exact T02 **56 nonempty suites /366 JVM identities** (T01 52/340 + T02 4/26). T03 changes no JVM test or production source. Freeze names before the committed-head full `--rerun-tasks` gate. The two new paired identities plus the affected prior RG7 pointer and RG9 gesture identities give **18 selected Android identities**. Fixture-server tests are a separate Python count, not JVM identities.
