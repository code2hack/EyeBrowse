...
Each native session has at most one live agent process and one lifecycle/controller owner. Paseo owns local Pi RPC lifecycle; create/import/restart must preserve in-flight work and history and verify that no competing runner remains. Hidden parallel/nested local agents, a second native-session process, and independent raw-stdin/RPC controllers are not permitted. Normal local messaging follows Section 15.3 through Paseo.
...
One native session or remote conversation has one coordinated request stream. Local Pi messages use the verified Paseo transport binding under Section 15.1 and Paseo's lifecycle owner under Section 8, not ordinary tmux send-keys, direct native RPC/stdin control, or another control plane. Check current assignment, native/Paseo binding, lifecycle and pending human gates before delivery; coordinate sends with retirement and reject stale assignment revisions at the receiver.
