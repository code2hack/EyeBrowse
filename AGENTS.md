...
Three completed unsuccessful Worker attempts on the same todo trigger Expert assignment. An unsuccessful attempt is one scope-bounded implementation or correction round concluded as `FAILED`, `NOT_PASSED`, or Reviewer `CHANGES_REQUESTED` for that objective. A helper report and review of the same round are one outcome, not two attempts. Multiple findings in one round count once per affected todo; do not charge unrelated todos for a ticket-wide verdict.
...
After the first unsuccessful Worker round, the Manager routes the findings for a second round. After the second unsuccessful Worker round, the Manager routes the findings for a third round. At the third unsuccessful round, the Worker stops independent correction of that todo and reports its approaches, failed criteria, current code/evidence, and safe state.
...
Later, distinct todos use their own three-failure Worker threshold. If the same resolved defect recurs or its acceptance is rejected later, reopen its history and return it to Expert without manufacturing three fresh Worker attempts. Todo completion, successful handback, and ticket/PR acceptance are distinct events.

### 14.5 Cutover from legacy ticket budgets

At rollout, preserve existing ticket-level Worker/Manager counts and evidence as history; new decisions use todo-level accounting and Expert escalation. Todos with attempt history at adoption time retain their recorded counts and proceed under the three-round threshold: recorded history is preserved, no counts are erased or invented, and mandatory Expert escalation triggers at the third concluded unsuccessful Worker round on the same todo. Map documented failures to the actual unresolved objective rather than automatically charging every todo or inventing fresh attempts. If attribution is ambiguous, retain that uncertainty and assign Expert assistance for the unresolved repeated failure instead of falsifying a count.
