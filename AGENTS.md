...
All active local agents, including the Manager, Planners, Workers, Experts, Reviewers, and helpers, MUST be Owner-visible and reachable as Paseo-managed instances in one EyeBrowse project. The Manager coordinates explicit creation/import and verifies native identity, project/workspace placement, and actual process CWD before assigning work. Local Worker/Expert/helper source CWDs are their isolated assigned worktrees, not a shared `main` checkout. Common project membership is grouping, not source or credential isolation.

Each native session has at most one live agent process and one lifecycle/controller owner. Paseo owns the local agent lifecycle/control path; create/import/restart must preserve in-flight work and history and verify that no competing runner remains. Hidden parallel/nested local agents, a second process for the same native session, and independent side-channel controllers are not permitted. Normal local messaging follows Section 15.3 through Paseo.

Approved browser-based ChatGPT agents and other supported remote roles retain their registered conversation/session surfaces; they are not represented as native Paseo imports without a verified supported path. A local helper remains a separate Paseo-managed Worker and does not replace the remote agent's identity.
...
The Manager MUST assign each local Worker, Expert, and helper an exact isolated worktree as its process CWD and a unique mission-owned temporary directory. Verify that its Paseo workspace resolves to that assigned source directory; a project/workspace label does not establish the CWD. Remote assignments follow Section 5.3. Concurrent writers use isolated worktrees/branches with coordinated source ownership; an Expert or helper takeover transfers the specific write assignment before edits begin.
...
For a sudo operation, the Manager MUST briefly explain the exact command and purpose and arrange an explicitly approved Owner-controlled interactive terminal procedure, qualified for private password entry without secret capture. The Owner enters any required password directly; agents do not supply or relay it through messages, tool arguments, logs, or repository files. A Paseo terminal may be used only after that path is verified; its availability alone is not qualification. If no safe interactive terminal is available, retain the human capability gate rather than bypassing authentication. Terminal recipes and qualification evidence belong in `DEV.md` and runtime records.
...
Worker attempts are accounted per todo; no fixed number of unsuccessful attempts triggers Expert assignment. An unsuccessful attempt is one scope-bounded implementation or correction round concluded as `FAILED`, `NOT_PASSED`, or Reviewer `CHANGES_REQUESTED` for that objective. A helper report and review of the same round are one outcome, not two attempts. Multiple findings in one round count once per affected todo; do not charge unrelated todos for a ticket-wide verdict.
...
After an unsuccessful Worker round, the Manager routes the findings for the next Worker round while the round's evidence supports continued Worker correction. The Manager MAY assign an Expert for the todo at any point, based on diagnosis complexity, repeated-failure patterns, risk, or resource considerations. Expert assignment is a Manager judgment, not an automatic consequence of a failure count. When the Manager assigns an Expert, the Worker stops independent correction of that todo and reports its approaches, failed criteria, current code/evidence, and safe state.
...
Later, distinct todos are accounted separately under Section 14.1. If the same resolved defect recurs or its acceptance is rejected later, reopen its history and return it to Expert under the Manager's judgment without manufacturing fresh Worker attempts. Todo completion, successful handback, and ticket/PR acceptance are distinct events.

### 14.5 Cutover from legacy ticket budgets

At rollout, preserve existing ticket-level Worker/Manager counts and evidence as history; new decisions use todo-level accounting and Expert escalation. Todos with attempt history at adoption time retain their recorded counts as history: recorded history is preserved, no counts are erased or invented, and Expert escalation follows this section's judgment-based rule with no automatic round-count trigger. Map documented failures to the actual unresolved objective rather than automatically charging every todo or inventing fresh attempts. If attribution is ambiguous, retain that uncertainty and consider Expert assistance for the unresolved repeated failure instead of falsifying a count.
...
One native session or remote conversation has one coordinated request stream. Local agent messages use the verified Paseo transport binding under Section 15.1 and Paseo's lifecycle owner under Section 8, not an independent side-channel controller. Check current assignment, native/Paseo binding, lifecycle and pending human gates before delivery; coordinate sends with retirement and reject stale assignment revisions at the receiver.
