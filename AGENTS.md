...
The Manager MUST confirm receipt and successful startup for all dispatched agents in the batch, including required local helpers. For local agents, verify the local-management binding, actual native session, process CWD, and effective Owner-selected model/effort; a created entry or title is not startup confirmation. A startup failure becomes a reported blocker. Once dispatch is complete and no actionable report remains pending, the Manager ends its turn and waits for reports.
...
Agent work has no active-work or wall-clock time ceiling and no clock-based checkpoint or permission-renewal requirement. Elapsed time or a transport wait timeout alone is not a failed attempt, cancellation, or authorization to resend. Event-driven reports and genuine stop conditions remain required. Finite individual command/test safety limits, product timing requirements, CPU/memory/concurrency limits, security gates, and todo failure accounting remain unchanged; bounded operations are not an overall agent work-time budget.
...
All active local agents, including the Manager, Planners, Workers, Experts, Reviewers, and helpers, MUST be Owner-visible and reachable through the approved local agent-management surface under one EyeBrowse project context. The Manager coordinates lifecycle operations such as creation or migration and verifies native identity, project/workspace placement, and actual process CWD before assigning work. Local Worker/Expert/helper source CWDs are their isolated assigned worktrees, not a shared `main` checkout. Common project membership is grouping, not source or credential isolation.

Each native session has at most one live agent process and one lifecycle/controller owner. The approved local agent-management layer owns that process's lifecycle and control path; creation, migration, and restart operations must preserve in-flight work and history and verify that no competing runner remains. Hidden parallel/nested local agents, a second process for the same native session, and independent side-channel controllers are not permitted. Normal local messaging follows Section 15.3 through the approved local transport.

Approved remote agents retain their registered remote conversation/session surfaces; they are not forced into the local agent-management surface without a verified supported path. A local helper remains a separate managed Worker and does not replace the remote agent's identity.
...
The Manager MUST assign each local Worker, Expert, and helper an exact isolated worktree as its process CWD and a unique mission-owned temporary directory. Verify that its managed workspace/instance binding resolves to that assigned source directory; a project/workspace label does not establish the CWD. Remote assignments follow Section 5.3. Concurrent writers use isolated worktrees/branches with coordinated source ownership; an Expert or helper takeover transfers the specific write assignment before edits begin.
...
For a sudo operation, the Manager MUST briefly explain the exact command and purpose and arrange an explicitly approved Owner-controlled interactive terminal procedure, qualified for private password entry without secret capture. The Owner enters any required password directly; agents do not supply or relay it through messages, tool arguments, logs, or repository files. A terminal exposed by the approved local management surface may be used only after that path is verified; its availability alone is not qualification. If no safe interactive terminal is available, retain the human capability gate rather than bypassing authentication. Terminal recipes and qualification evidence belong in `DEV.md` and runtime records.
...
Post-merge cleanup follows Section 13; agent retirement/archival follows Section 15.1. Worktree removal and destructive shared-Git operations require approval for those specific operations. Agent archival is not workspace deletion or worktree-removal permission. Exact workspace, resource, and interactive-terminal procedures belong in `DEV.md`, subject to Section 3.
...
After merge, the Manager MUST coordinate cleanup with the ticket Worker and any remaining assigned Expert/helpers, verify the cleanup reports and retained evidence, release resources, and complete Section 15.1 retirement/archival for ticket-scoped participants with no remaining assignment before recording the final result and closing the issue. Preserve or transfer still-active child assignments before archiving their parent; runtime parentage is not permission to terminate unrelated work. Experts normally finish their scoped assignments at todo handback under Section 14.4. If an agent is unavailable, arrange scoped replacement cleanup and record the handoff. Cleanup and archival remain pending until verified.
...
Two completed unsuccessful Worker attempts on the same todo trigger Expert assignment. An unsuccessful attempt is one scope-bounded implementation or correction round concluded as `FAILED`, `NOT_PASSED`, or Reviewer `CHANGES_REQUESTED` for that objective. A helper report and review of the same round are one outcome, not two attempts. Multiple findings in one round count once per affected todo; do not charge unrelated todos for a ticket-wide verdict.

Expected red tests and intermediate debugging failures within a round are not separate attempts. An established external infrastructure/access/human blocker is reported as blocked, not an implementation failure. An incomplete required deliverable without an established external cause remains not passed when its round concludes. Each round has declared scope and resource limits. Record an actually concluded outcome without concealing it as intermediate work; elapsed agent work time does not itself conclude a round or impose a checkpoint. Section 6.3 distinguishes command/product timing from agent work duration.
...
Preserve Expert evidence, clean up or transfer its assigned resources, and complete Section 15.1 retirement/archival for the Expert and any Expert-only helper when their assignment ends. Preserve still-assigned children and pending remote-conversation work before parent archival. Registry retirement or archival does not authorize deleting conversation history, browser profiles, or credentials. A local helper serving as the ticket Worker returns to its normal Worker duties rather than being retired or archived. Retaining an Expert for a further assignment requires an explicit Manager assignment within approved authority, not automatic takeover of the ticket.
...
For every managed local instance, maintain a verified runtime binding from its native registry tuple to the approved local management surface, including its connection endpoint, instance ID, EyeBrowse project/workspace placement, native session record, assigned source CWD, effective model/effort, and lifecycle. Reconcile the binding after lifecycle migration or recreation before routing. Keep these transport details outside the unchanged eight-field registry; display titles and management-surface IDs alone do not grant authority.

Retirement is a coordinated lifecycle operation: block new work routing and mark retiring in runtime; safely settle/cancel or hand off assigned operations; preserve history, evidence, pending requests and remote-conversation references; complete cleanup and resource/write-ownership obligations; detach or transfer still-active child agents and verify their continued assignments; then perform the approved retirement/archive operation for the exact managed local agent and verify both its terminal lifecycle state and cessation of its agent runtime before recording registry retirement/closeout. Parent retirement/archival must not stop still-assigned children. If that preservation cannot be verified, keep retirement/archival pending and report the blocker.

Retain read-only contributor and transport history. Agent retirement/archive is not deletion, ticket acceptance, workspace removal, or permission to reactivate. Ordinary messages must not resume retired/retiring/archived agents; reactivation requires an explicit assignment within approved authority and verified lifecycle/ownership reconciliation. Preserve remote conversation history under its approved procedure rather than forcing it into the local management surface. Previously retired native histories need not be relaunched or migrated merely for cosmetic archival; dispose of any remaining legacy process only through coordinated safe retirement.

A Manager handoff leaves one active project-wide Manager and transfers run/request state, remote-conversation ownership and still-assigned children before retiring/archiving the outgoing Manager. Verify the successor's receipt, current bindings and children's continued operation before outgoing archival; no parallel coordinator or controller is created. Communicate the new routing identity to affected agents. Registration records authority already granted; it does not itself grant authority or access.
...
Use stable message IDs to correlate receipt and reply; retries of the same logical send retain its identifier and unchanged body. A conflicting payload is an error to reconcile, not a duplicate to execute. Replies reference the request being answered. Include applicable ticket/todo, attempt, assignment revision, candidate, and evidence references in the payload. Record send intent, transport dispatch acceptance, recipient receipt, complete final report, evidence verification, and acceptance as distinct facts. A delayed or duplicate reply or a truncated finish notification must not create another dispatch, attempt, approval, or source handoff.

One native session or remote conversation has one coordinated request stream. Local agent messages use the verified local transport binding under Section 15.1 and the lifecycle owner under Section 8, not an independent side-channel controller. Check current assignment, native/local-management binding, lifecycle and pending human gates before delivery; coordinate sends with retirement and reject stale assignment revisions at the receiver.

Routine delivery must preserve ongoing work and pending Owner permission/authentication prompts. Use a qualified explicit busy-delivery policy; defer ordinary messages when that policy cannot preserve a busy turn or a pending gate. Reconcile the gate through the authorized human path before routine delivery. A transport default that interrupts, clears permissions or unarchives is not authority to do so. Intentional cancellation, restart or reactivation requires the applicable explicit lifecycle decision; SDK/command choices and installed-version caveats belong in `DEV.md`.
...
`DEV.md` defines approved development procedures, runtime/model policy, build/test/device workflows and connection priority, local agent-management project/workspace/lifecycle procedures, qualified local messaging, safe interactive terminals, retirement/archival, and references to the installed todo, remote-agent, device-screen, and Owner-messaging integrations. It implements remote/local handoffs, alarm/sudo procedures, and recovery while referencing this file for authority, acceptance, escalation, and canonical message semantics. Commands and reusable operational detail belong in the verified skills/scripts; addresses, private credential references, and live assignments belong in local runtime. Avoid independently editable copies of the same rule or setting. The Manager distinguishes verified facts, proposed procedures, and unresolved limitations.

Harness/runtime configuration holds executable settings, installed integration versions, and provider mappings; secret stores or environment configuration hold credentials. Actual todo IDs/counts, assignments, Expert return targets, remote/helper pairing, discovered addresses, request state, run-specific Owner choices, and test outcomes belong in durable todo/mission/runtime/evidence records. Reuse the installed todo tool where it preserves the required state; reference or supplement missing metadata in existing run records without creating a competing source of truth. The approved local management layer owns local agent process/control lifecycle; the native runtime retains session history; runtime records bind project authority, assignments and transport state; the approved local messaging path performs delivery; the Manager coordinates and verifies outcomes. The owning procedure or loader must consume the records: editing a configuration file alone is not execution.

Changes to tools, scripts, or configuration remain within approved scope and authority; moving a rule out of a protected article does not itself authorize changing that rule. A successful skill-loader check or one transport/helper smoke test is evidence only for what it exercised, not proof of end-to-end orchestration. Before relying on affected operations, qualify project/workspace/CWD binding, single-runner import/restart, busy and pending-permission delivery, duplicate/conflicting/uncertain sends, complete-result recovery, retirement/child preservation, safe interactive terminals, escalation, local verification and Expert-to-Worker return. A basic message round trip does not qualify restart/archive/race behavior. Use labeled setup cases rather than product retries, fake contributor registrations, or destructive tests on active product agents. Stage changes, preserve in-flight operations and remote browser references, and reconcile/restart affected instances through their lifecycle owner at safe points; remaining capability gaps block only dependent operations unless a genuine broader gate applies. Publication of governance does not itself activate a runtime or resume Owner-paused work. This file does not require converting `DEV.md` to another format.
