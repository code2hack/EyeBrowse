...
- accepts debug-only commands from `adb shell` through a receiver protected by `android.permission.DUMP`; the receiver is declared only in `app/src/debug/AndroidManifest.xml` and is absent from release APKs.
...
Debug builds include the command receiver; release builds do not. Verify that boundary and run the
telemetry-encoding regression tests with:

```bash
ANDROID_HOME=/path/to/Android/Sdk ./verify-variants.sh
ANDROID_HOME=/path/to/Android/Sdk ./gradlew :app:testDebugUnitTest :app:lintDebug
```
...
`telemetry.jsonl` records Android's `interactive`, `deviceLocked`, `batteryPct`, `batteryStatus`, and `plugged` values on every persisted event. String values are encoded with `org.json.JSONObject.quote`, which escapes the full U+0000-U+001F control range (the earlier hand-rolled encoder emitted raw TAB/backspace/form-feed and produced invalid JSONL).
...
The receiver is exported only for this debug spike and requires the privileged `DUMP` permission, which `adb shell` has. It is present only in debug builds; release APKs have no command surface.
